...
tempDat <- s1_4
tempDat <- tempDat[,c(2,4,10,7)]
head(tempDat)
dat <- tempDat
dat <- na.omit(dat)

#Check for repeating gene-pairs and how many
dat$allele_pair <- paste(dat$`Query allele name`,dat$`Array allele name`,sep = "_")
allele_pair_counts <- table(dat$allel_pair)
dim(gene_pair_counts[gene_pair_counts > 1])

# Aggregate the growth score for each gene pair
aggregated_growth <- aggregate(dat$`Double mutant fitness`, by = list(dat$allel_pair), FUN = sum)
...
weighted_pvalue <- tapply(dat$`P-value`* adjusted_pvalues,
                          dat$allel_pair, sum) / tapply(adjusted_pvalues, 
                                                        dat$allel_pair, sum)
...
condensed_data <- data.table(
  allele_pair = unique(dat$allel_pair),
